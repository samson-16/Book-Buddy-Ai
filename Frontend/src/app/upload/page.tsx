...
export default function UploadSummarize() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // For MVP, send text only (later we’ll parse PDF/TXT on backend)
      const response = await fetch("http://127.0.0.1:8000/summarize/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          text: text || "Default sample text",
          max_length: length === "short" ? 80 : length === "medium" ? 150 : 250,
          min_length: 30,
        }),
      });
  ...
  return (
    <div className="min-h-screen  bg-gray-100 py-6 px-10 flex justify-center items-center">
      <div className="w-full max-w-4xl">
        ...
        >
           <div>
             <h1 className="text-2xl font-merriweather font-bold text-
